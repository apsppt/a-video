...
fn get_db_path() -> Result<String, rusqlite::Error> {
    // 使用应用可执行文件所在目录作为数据库路径
    let exe_path = std::env::current_exe()
        .map_err(|e| rusqlite::Error::InvalidPath(format!("无法获取应用路径: {}", e).into()))?;
    
    // 获取应用所在目录
    let app_dir = exe_path.parent()
        .ok_or_else(|| rusqlite::Error::InvalidPath("无法获取应用目录".to_string().into()))?;
    
    let mut db_path = app_dir.to_path_buf();
    db_path.push("video.db");
    ...
    db_path.to_str()
        .map(|s| s.to_string())
        .ok_or_else(|| rusqlite::Error::InvalidPath("数据库路径无效".to_string().into()))
}
...
fn init_database_safely() -> SqliteDB {
    // 首先尝试使用文件数据库
    if let Ok(path) = get_db_path() {
        if let Ok(db) = SqliteDB::new(&path) {
            // 尝试初始化表，失败也没关系
    ...
    // 文件数据库失败，尝试内存数据库
    if let Ok(db) = SqliteDB::new(":memory:") {
        let _ = db.init_tables();
        return db;
    }
    
    // 所有方法都失败，创建一个安全的默认实例
    // 这个实例可能无法正常工作，但应用不会崩溃
    eprintln!("警告：所有数据库初始化方法都失败，使用安全模式");
    
    // 使用一个完全安全的fallback，避免任何panic
    let safe_connection = Connection::open(":memory:")
    ...
    SqliteDB {
        conn: Arc::new(Mutex::new(safe_connection))
    }
}
